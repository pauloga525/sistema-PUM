    ...
    <div className="min-h-screen bg-pum-bg flex flex-col">
      ...
      <header
        ...
        style={{
          ...
          boxShadow: "0 1px 0 rgba(255,255,255,0.08), 0 4px 24px rgba(30,10,60,0.35)",
        }}
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:block">
            ...
            <p className="text-purple-200/60 text-[11px] leading-none mt-0.5">Panel de Sistema</p>
          </div>
        </div>

        {/* Lado derecho: badge + usuario + logout */}
        <div className="flex items-center gap-3">
          <span
            className="text-[10px] font-bold px-2 py-0.5 rounded-full tracking-wide select-none"
            style={{ background: "#a78bfa", color: "#1e0a3c" }}
          >
            SUPERADMIN
          </span>
          ...
          <div className="text-right hidden sm:block">
            <p className="text-white text-sm font-medium leading-tight">
              {session.user?.name ?? "Super Administrador"}
            </p>
            <p className="text-purple-200/55 text-[11px] leading-none mt-0.5">{session.user?.email}</p>
          </div>
          ...
          <form action={async () => {
            ...
            <button
              type="submit"
              title="Cerrar sesión"
              className="pum-topbar-btn flex items-center gap-1.5 text-[12px] font-medium text-purple-100/70 hover:text-white px-2.5 py-1.5 rounded-lg cursor-pointer"
            >
